...
class BaseApplication(BaseModel):
    ...
    move_in_month: str = Field("", min_length=1, max_length=2)
    ...
    # 既存ライブラリは存在しますが、既存ライブラリでは、windowsの項目が入らないので自作のバリデーションをかけます。
    # 上記項目が入らない理由については調査中です。原因がわかり次第、ライブラリに移行します。
    # ライブラリURL → https://pypi.org/project/user-agents/
    @validator("user_agent")
    def user_agent_validation(v):
        v = v.replace(" ", "")
        permission_pattern = "^[A-Za-z0-9\\!\"#$%&'()-^@[;:,./=~_]+$"
        if not re.match(permission_pattern, v):
            raise ValueError("入力された値が許容された『半角英数記号』ではありません。")
        return v
    ...
    def move_in_year_validation(v):
        this_year = int(datetime.now().strftime("%Y"))

        if not re.match("^[0-9]{4}$", v):
            raise ValueError("入居年は半角数字で入力してください。")
        else:
            if int(v) < 1000:
                raise ValueError("入居年が1000年より前です。")
    ...
    def hire_date_year_validation(cls, v, values):
        ...
        if not re.match("^[0-9]{4}$", v):
            raise ValueError("入社年は半角数字で入力してください。")
        else:
    ...
    def parse_kana_name(self):
        fixed_kana_last_name = mojimoji.zen_to_han(self.kana_last_name)[:28]
        fixed_kana_first_name = mojimoji.zen_to_han(self.kana_first_name)
        total_chars = len(fixed_kana_last_name + fixed_kana_first_name)

        if total_chars > 29:
            delete_chars = 29 - len(fixed_kana_last_name)
            fixed_kana_first_name = fixed_kana_first_name[:delete_chars]
        return fixed_kana_last_name, fixed_kana_first_name
    ...
    def parse_move_in_date(self):
        if self.move_in_year == self.birthday_year:
            self.move_in_month = self.birthday_month
        elif self.move_in_year < self.birthday_year:
            self.move_in_year = self.birthday_year
            self.move_in_month = self.birthday_month
        else:
            # 　ダミー値を代入する
            self.move_in_month = "01"
        return self.move_in_year, self.move_in_month
